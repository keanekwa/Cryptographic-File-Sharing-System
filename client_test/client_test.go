...
var _ = Describe("Client Tests", func() {
	...
	Describe("Basic Tests", func() {
		...
		Specify("Basic Test: 2 users creating files with the same file name", func() {
		...
		Specify("Basic Test: Accept invitation from the wrong person.", func() {
			userlib.DebugMsg("Initializing Alice and Bob")
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Basic Test: Loadfile before accepting invitation.", func() {
			userlib.DebugMsg("Initializing Alice and Bob")
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
			...
			userlib.DebugMsg("Bob loads file with filename %s.", aliceFile)
			_, err = bob.LoadFile(aliceFile)
			Expect(err).ToNot(BeNil())

			userlib.DebugMsg("Bob loads file with filename %s.", bobFile)
			_, err = bob.LoadFile(bobFile)
			Expect(err).ToNot(BeNil())
		})
		...
		Specify("Basic Test: User who is not an owner tries to revoke access", func() {
			userlib.DebugMsg("Initializing Alice, Bob and Charles")
			alice, err = client.InitUser("alice", defaultPassword)
			Expect(err).To(BeNil())
		...
		Specify("Basic Test: Fuzz Testing", func() {
			...
			userlib.DebugMsg("Charles tries to append to file: %s", bobFile)
			err = charles.AppendToFile(bobFile, []byte(contentThree))
			Expect(err).ToNot(BeNil())

			userlib.DebugMsg("Bob creating invite for himself.")
			invite, err = bob.CreateInvitation(bobFile, "bob")
			Expect(err).ToNot(BeNil())
			...
			userlib.DebugMsg("Charles tries to append to file: %s", charlesFile)
		...
		Specify("Basic Test: Do not allow invite if file has not been shared with a user.", func() {
